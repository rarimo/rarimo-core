...
import (
	...
	"gitlab.com/rarimo/rarimo-core/ethmintrpc"
...
func StartJSONRPC(ctx *server.Context,
	...
	apis := ethmintrpc.GetRPCAPIs(ctx, clientCtx, tmWsClient, allowUnprotectedTxs, indexer, rpcAPIArr)
	...
	tmWsClient = ConnectTmWS(tmRPCAddr, tmEndpoint, ctx.Logger)
	wsSrv := ethmintrpc.NewWebsocketsServer(clientCtx, ctx.Logger, tmWsClient, config)
	wsSrv.Start()
