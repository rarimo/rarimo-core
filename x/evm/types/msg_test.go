...
func (suite *MsgsTestSuite) SetupTest() {
	from, privFrom := ethminttests.NewAddrKey()

	suite.signer = ethminttests.NewSigner(privFrom)
	suite.from = from
	suite.to = ethminttests.GenerateAddress()
...
func (suite *MsgsTestSuite) TestMsgEthereumTx_BuildTx() {
	...
	for _, tc := range testCases {
		...
		tx, err := tc.msg.BuildTx(suite.clientCtx.TxConfig.NewTxBuilder(), "astake")
		if tc.expError {
			...
			suite.Require().Equal(sdk.NewCoins(sdk.NewCoin("astake", sdkmath.NewInt(100000))), tx.GetFee())
		}
	}
}
