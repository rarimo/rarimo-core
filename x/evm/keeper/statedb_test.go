...
func (suite *KeeperTestSuite) TestGetCodeHash() {
	addr := ethminttests.GenerateAddress()
...
func (suite *KeeperTestSuite) TestExist() {
	testCases := []struct {
		...
		{"success, account doesn't exist", ethminttests.GenerateAddress(), func(vm.StateDB) {}, false},
	}
...
func (suite *KeeperTestSuite) TestEmpty() {
	testCases := []struct {
		...
		{"empty, account doesn't exist", ethminttests.GenerateAddress(), func(vm.StateDB) {}, true},
	}
...
func (suite *KeeperTestSuite) CreateTestTx(msg *types.MsgEthereumTx, priv cryptotypes.PrivKey) authsigning.Tx {
	...
	err = msg.Sign(suite.ethSigner, ethminttests.NewSigner(priv))
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestAddLog() {
	addr, privKey := ethminttests.NewAddrKey()
...
func (suite *KeeperTestSuite) TestPrepareAccessList() {
	dest := ethminttests.GenerateAddress()
	precompiles := []common.Address{ethminttests.GenerateAddress(), ethminttests.GenerateAddress()}
	accesses := ethtypes.AccessList{
		{Address: ethminttests.GenerateAddress(), StorageKeys: []common.Hash{common.BytesToHash([]byte("key"))}},
		{Address: ethminttests.GenerateAddress(), StorageKeys: []common.Hash{common.BytesToHash([]byte("key1"))}},
	}
...
func (suite *KeeperTestSuite) AddSlotToAccessList() {
	testCases := []struct {
		...
		{"new address and slot (1)", ethminttests.GenerateAddress(), common.BytesToHash([]byte("hash"))},
