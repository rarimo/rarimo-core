...
func SimulateMsgUpdateConfirmation(
	ak types.AccountKeeper,
	bk types.BankKeeper,
	k keeper.Keeper,
) simtypes.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		var (
			...
			msg             = &types.MsgUpdateConfirmation{}
			allConfirmation = k.GetAllConfirmation(ctx)
			found           = false
		)
		for _, obj := range allConfirmation {
		...
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "confirmation creator not found"), nil, nil
		}
		msg.Creator = simAccount.Address.String()

		msg.Height = confirmation.Height
...
func SimulateMsgDeleteConfirmation(
	ak types.AccountKeeper,
	bk types.BankKeeper,
	k keeper.Keeper,
) simtypes.Operation {
