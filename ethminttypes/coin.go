...
const (
	// AttoPhoton is the alias on `stake` denom and defines the default coin denomination used in Ethermint in:
	//
...
// NewPhotonDecCoin is a utility function that returns an "astake" decimal coin with the given sdkmath.Int amount.
// The function will panic if the provided amount is negative.
func NewPhotonDecCoin(amount sdkmath.Int) sdk.DecCoin {
	return sdk.NewDecCoin(AttoPhoton, amount)
}

// NewPhotonCoinInt64 is a utility function that returns an "astake" coin with the given int64 amount.
// The function will panic if the provided amount is negative.
