...
func (suite *BackendTestSuite) TestTraceTransaction() {
	...
	tx, _ := msgEthereumTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), "astake")
	txBz, _ := txEncoder(tx)

	msgEthereumTx2.From = from.String()
	msgEthereumTx2.Sign(ethSigner, suite.signer)
	tx2, _ := msgEthereumTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), "astake")
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("case %s", tc.name), func() {
			...
			db := dbm.NewMemDB()
			suite.backend.indexer = ethmintindexer.NewKVIndexer(db, tmlog.NewNopLogger(), suite.backend.clientCtx)
