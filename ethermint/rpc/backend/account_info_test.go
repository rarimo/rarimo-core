...
func (suite *BackendTestSuite) TestGetCode() {
	blockNr := types.NewBlockNumber(big.NewInt(1))
	...
	testCases := []struct {
		...
		},
		{
			...
			ethminttests.GenerateAddress(),
			types.BlockNumberOrHash{BlockNumber: &blockNr},
			func(addr common.Address) {
...
func (suite *BackendTestSuite) TestGetProof() {
	blockNrInvalid := types.NewBlockNumber(big.NewInt(1))
	blockNr := types.NewBlockNumber(big.NewInt(4))
	address1 := ethminttests.GenerateAddress()
	...
	testCases := []struct {
		...
		storageKeys   []string
		blockNrOrHash types.BlockNumberOrHash
		registerMock  func(types.BlockNumber, common.Address)
		expPass       bool
		expAccRes     *types.AccountResult
	}{
		{
			"fail - BlockNumeber = 1 (invalidBlockNumber)",
			address1,
			[]string{},
			types.BlockNumberOrHash{BlockNumber: &blockNrInvalid},
			func(bn types.BlockNumber, addr common.Address) {
		...
		},
		{
			...
			types.BlockNumberOrHash{BlockNumber: &blockNr},
			func(bn types.BlockNumber, addr common.Address) {
				suite.backend.ctx = types.ContextWithHeight(bn.Int64())
...
func (suite *BackendTestSuite) TestGetTransactionCount() {
	testCases := []struct {
		...
		registerMock func(common.Address, types.BlockNumber)
		expPass      bool
	...
	}{
		{
			...
			false,
			types.NewBlockNumber(big.NewInt(1)),
			func(addr common.Address, bn types.BlockNumber) {
		...
		},
		{
			...
			false,
			types.NewBlockNumber(big.NewInt(10000)),
			func(addr common.Address, bn types.BlockNumber) {
				var header metadata.MD
				queryClient := suite.backend.queryClient.QueryClient.(*mocks2.EVMQueryClient)
				RegisterParams(queryClient, &header, 1)
