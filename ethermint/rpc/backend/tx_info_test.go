...
func (suite *BackendTestSuite) TestGetTransactionByHash() {
	...
	rpcTransaction, _ := types2.NewRPCTransaction(msgEthereumTx.AsTransaction(), common.Hash{}, 0, 0, big.NewInt(1), suite.backend.chainID)

	testCases := []struct {
		name         string
		registerMock func()
		tx           *evmtypes.MsgEthereumTx
		expRPCTx     *types2.RPCTransaction
		expPass      bool
	}{
		{
			"fail - Block error",
...
func (suite *BackendTestSuite) TestGetTransactionByBlockAndIndex() {
	...
	txFromMsg, _ := types2.NewTransactionFromMsg(
		msgEthTx,
...
func (suite *BackendTestSuite) TestQueryTendermintTxIndexer() {
	testCases := []struct {
		name         string
		registerMock func()
		txGetter     func(*types2.ParsedTxs) *types2.ParsedTx
		query        string
		expTxResult  *ethermint.TxResult
		expPass      bool
	}{
		{
			...
			func(txs *types2.ParsedTxs) *types2.ParsedTx {
				return &types2.ParsedTx{}
			},
			"",
			&ethermint.TxResult{},
			false,
