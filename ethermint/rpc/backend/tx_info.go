...
func (b *Backend) GetTransactionByHash(txHash common.Hash) (*types.RPCTransaction, error) {
	...
	block, err := b.TendermintBlockByNumber(types.BlockNumber(res.Height))
	if err != nil {
		return nil, err
	}
...
func (b *Backend) getTransactionByHashPending(txHash common.Hash) (*types.RPCTransaction, error) {
	hexTx := txHash.Hex()
...
func (b *Backend) GetTransactionReceipt(hash common.Hash) (map[string]interface{}, error) {
	...
	resBlock, err := b.TendermintBlockByNumber(types.BlockNumber(res.Height))
	if err != nil {
...
func (b *Backend) GetTransactionByBlockHashAndIndex(hash common.Hash, idx hexutil.Uint) (*types.RPCTransaction, error) {
...
func (b *Backend) GetTransactionByBlockNumberAndIndex(blockNum types.BlockNumber, idx hexutil.Uint) (*types.RPCTransaction, error) {
...
func (b *Backend) queryTendermintTxIndexer(query string, txGetter func(*types.ParsedTxs) *types.ParsedTx) (*ethermint.TxResult, error) {
	...
	if !types.TxSuccessOrExceedsBlockGasLimit(&txResult.TxResult) {
		return nil, errors.New("invalid ethereum tx")
	}
