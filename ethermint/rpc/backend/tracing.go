...
func (b *Backend) TraceTransaction(hash common.Hash, config *evmtypes.TraceConfig) (interface{}, error) {
	...
	blk, err := b.TendermintBlockByNumber(types.BlockNumber(transaction.Height))
	if err != nil {
	...
	traceResult, err := b.queryClient.TraceTx(types.ContextWithHeight(contextHeight), &traceTxRequest)
	if err != nil {
		return nil, err
	}
...
func (b *Backend) TraceBlock(height types.BlockNumber,
	...
	for i, tx := range txs {
		if !types.TxSuccessOrExceedsBlockGasLimit(blockRes.TxsResults[i]) {
	...
	ctxWithHeight := types.ContextWithHeight(int64(contextHeight))
