...
func (suite *BackendTestSuite) TestResend() {
	...
	toAddr := ethminttests.GenerateAddress()
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				client := suite.backend.clientCtx.Client.(*mocks2.Client)
				queryClient := suite.backend.queryClient.QueryClient.(*mocks2.EVMQueryClient)
				feeMarketClient := suite.backend.queryClient.FeeMarket.(*mocks2.FeeMarketQueryClient)
				RegisterParams(queryClient, &header, 1)
...
func (suite *BackendTestSuite) TestSendRawTransaction() {
	...
	cosmosTx, _ := ethTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), "astake")
