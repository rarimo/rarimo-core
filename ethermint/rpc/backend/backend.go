...
type EVMBackend interface {
	...
	GetBlockByNumber(blockNum types2.BlockNumber, fullTx bool) (map[string]interface{}, error)
	GetBlockByHash(hash common.Hash, fullTx bool) (map[string]interface{}, error)
	GetBlockTransactionCountByHash(hash common.Hash) *hexutil.Uint
	GetBlockTransactionCountByNumber(blockNum types2.BlockNumber) *hexutil.Uint
	TendermintBlockByNumber(blockNum types2.BlockNumber) (*tmrpctypes.ResultBlock, error)
	...
	BlockNumberFromTendermint(blockNrOrHash types2.BlockNumberOrHash) (types2.BlockNumber, error)
	...
	HeaderByNumber(blockNum types2.BlockNumber) (*ethtypes.Header, error)
	...
	EthBlockByNumber(blockNum types2.BlockNumber) (*ethtypes.Block, error)
	EthBlockFromTendermintBlock(resBlock *tmrpctypes.ResultBlock, blockRes *tmrpctypes.ResultBlockResults) (*ethtypes.Block, error)

	// Account Info
	GetCode(address common.Address, blockNrOrHash types2.BlockNumberOrHash) (hexutil.Bytes, error)
	GetBalance(address common.Address, blockNrOrHash types2.BlockNumberOrHash) (*hexutil.Big, error)
	GetStorageAt(address common.Address, key string, blockNrOrHash types2.BlockNumberOrHash) (hexutil.Bytes, error)
	GetProof(address common.Address, storageKeys []string, blockNrOrHash types2.BlockNumberOrHash) (*types2.AccountResult, error)
	GetTransactionCount(address common.Address, blockNum types2.BlockNumber) (*hexutil.Uint64, error)
	...
	FeeHistory(blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*types2.FeeHistoryResult, error)
	SuggestGasTipCap(baseFee *big.Int) (*big.Int, error)

	// Tx Info
	GetTransactionByHash(txHash common.Hash) (*types2.RPCTransaction, error)
	GetTxByEthHash(txHash common.Hash) (*types.TxResult, error)
	GetTxByTxIndex(height int64, txIndex uint) (*types.TxResult, error)
	GetTransactionByBlockAndIndex(block *tmrpctypes.ResultBlock, idx hexutil.Uint) (*types2.RPCTransaction, error)
	GetTransactionReceipt(hash common.Hash) (map[string]interface{}, error)
	GetTransactionByBlockHashAndIndex(hash common.Hash, idx hexutil.Uint) (*types2.RPCTransaction, error)
	GetTransactionByBlockNumberAndIndex(blockNum types2.BlockNumber, idx hexutil.Uint) (*types2.RPCTransaction, error)
	...
	EstimateGas(args evmtypes.TransactionArgs, blockNrOptional *types2.BlockNumber) (hexutil.Uint64, error)
	DoCall(args evmtypes.TransactionArgs, blockNr types2.BlockNumber) (*evmtypes.MsgEthereumTxResponse, error)
	...
	TraceBlock(height types2.BlockNumber, config *evmtypes.TraceConfig, block *tmrpctypes.ResultBlock) ([]*evmtypes.TxTraceResult, error)
}
...
type Backend struct {
	...
	queryClient         *types2.QueryClient // gRPC query client
	logger              log.Logger
	chainID             *big.Int
	cfg                 config.Config
