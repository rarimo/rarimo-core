...
func (suite *BackendTestSuite) SetupTest() {
	...
	suite.acc = sdk.AccAddress(ethminttests.GenerateAddress().Bytes())
	...
	suite.backend.queryClient.QueryClient = mocks2.NewEVMQueryClient(suite.T())
	suite.backend.clientCtx.Client = mocks2.NewClient(suite.T())
	suite.backend.queryClient.FeeMarket = mocks2.NewFeeMarketQueryClient(suite.T())
	suite.backend.ctx = types.ContextWithHeight(1)
...
func (suite *BackendTestSuite) signAndEncodeEthTx(msgEthereumTx *evmtypes.MsgEthereumTx) []byte {
	from, priv := ethminttests.NewAddrKey()
	signer := ethminttests.NewSigner(priv)
	...
	tx, err := msgEthereumTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), "astake")
	suite.Require().NoError(err)
