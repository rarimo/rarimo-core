...
func TestIsEmptyHash(t *testing.T) {
	...
	for _, tc := range testCases {
		require.Equal(t, tc.expEmpty, types2.IsEmptyHash(tc.hash), tc.name)
	}
}
...
func TestIsZeroAddress(t *testing.T) {
	...
	for _, tc := range testCases {
		require.Equal(t, tc.expEmpty, types2.IsZeroAddress(tc.address), tc.name)
	}
}
...
func TestValidateAddress(t *testing.T) {
	testCases := []struct {
		...
		{
			"valid address", ethminttests.GenerateAddress().Hex(), false,
		},
	}

	for _, tc := range testCases {
		err := types2.ValidateAddress(tc.address)
...
func TestValidateNonZeroAddress(t *testing.T) {
	...
	for _, tc := range testCases {
		err := types2.ValidateNonZeroAddress(tc.address)
...
func TestSafeInt64(t *testing.T) {
	...
	for _, tc := range testCases {
		value, err := types2.SafeInt64(tc.value)
		if tc.expError {
			require.Error(t, err, tc.name)
			continue
		}
