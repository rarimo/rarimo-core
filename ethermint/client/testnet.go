...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		if err := network2.WriteFile(fmt.Sprintf("%v.json", "key_seed"), nodeDir, cliPrint); err != nil {
			return err
		}

		accStakingTokens := sdk.TokensFromConsensusPower(5000, types2.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(types2.AttoPhoton, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(types2.AttoPhoton, valTokens),
		...
		if err := network2.WriteFile(fmt.Sprintf("%v.json", nodeDirName), gentxsDir, txBz); err != nil {
			return err
		}

		customAppTemplate, customAppConfig := config.AppConfig(types2.AttoPhoton)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, types2.AttoPhoton, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
...
func startTestnet(cmd *cobra.Command, args startArgs) error {
	networkConfig := network2.DefaultConfig()
	...
	networkLogger := network2.NewCLILogger(cmd)
	...
	testnet, err := network2.New(networkLogger, baseDir, networkConfig)
	if err != nil {
		return err
	}
